...
//
{
    var BindInvoiceList = function () {
        ...
        $('#tbl_Buyerlist').DataTable({
            'bServerSide': true,
            "pagingType": "full_numbers",
            'bPaginate': true,
            // "scrollY": tableAvailabeHeight + "px",
            "ajax": {
                "url": "InvoiceBuyer360.aspx/GetInvoicelst",
                ...
                "dataSrc": function (json) {
                    ...
                    json.data = common.AUF(objData['BuyerList']);
            ...
            'aoColumns': [
                ...
                },
                {
                    ...
                    "mData": function (data, type, dataToSet) {
                        var html = '';
                        html += '<h5 >' + (data["ContactNumber"] == null || data["ContactNumber"] == '' ? '-' : data["ContactNumber"]) + '</h5>';
                        return html;
                    }
                },
                {
                    ...
                    "mData": function (data, type, dataToSet) {
                        var html = '';
                        html += '<h5 >' + (data["Courieraddress"] == null || data["Courieraddress"] == '' ? '-' : data["Courieraddress"]) + '</h5>';
                        return html;
                    }
                }, {
                    "width": '10%',
                    "mData": function (data, type, dataToSet) {
                        var html = '';
                        html += '<h5 >' + (data["Liners"] == null || data["Liners"] == '' ? '-' : data["Liners"]) + '</h5>';
                        return html;
                    }
                },
                {
                    ...
                    "mData": function (data, type, dataToSet) {
                        ...
                        html += '<a class="isc-action-badge-td-s1 pad-lft-5 btn_View" title="View" id="btn_View"  href="ViewBuyer360.aspx?BuyerID=' + (data["BuyerID"] == null ? '' : data["BuyerID"]) + '"><i class="fa fa-eye"></i></a>';
                        html += '<a class="isc-action-badge-td-s1 pad-lft-5" title="Edit"  href="AddBuyer.aspx?BuyerID=' + (data["BuyerID"] == null ? '' : data["BuyerID"]) + '"><i class="fa fa-pencil-square-o"></i></a>';
                        html += '<a class="isc-action-badge-td-s1 pad-lft-5 isc-mb-mar-lft-5" title="Delete" data-delete-buyer=' + (data["BuyerID"] == null ? '' : data["BuyerID"]) + '><i class="fa fa-trash-o"></i></a>';
                        html += '</div>';
                        return html;
    ...
    var ConfigureOrderBy = function (objModel) {
        ...
        switch (columnName) {
            ...
            case 1:
                ...
                break;
            case 2:
                orderBy = "ContactNumber " + direction;
                break;
            case 3:
                orderBy = "Courieraddress " + direction;
                break;
            case 4:
                orderBy = "Liners " + direction;
                break;
        }
        return orderBy;
    }
...
//
{
    ...
    var DeleteBuyer = function () {
        ...
        var tempList = {};
        $.when(RequestServer("InvoiceBuyer360.aspx/DeleteBuyer", obj)).done(function (response) {
            tempList = $.parseJSON(response);
            if (parseInt(response) > 0) {
                deleteBuyerID = 0;
