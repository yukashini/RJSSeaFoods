...
//DOM
{
    var BindCreateInvoiceScreen = function () {
        ...
        BindDropDowns($('#slt-Loading'), masterData["PortofLoading"], 'Choose Port of Loading')
        BindDropDowns($('#slt-Discharge'), masterData["PortofDischarge"], 'Choose Port of Discharge')
    ...
    var BindEditShipperData = function (ShipperData) {
        debugger
        var a = ShipperData[0]['ShipperAddress'];
        var b = ShipperData[0]['ShipperCity'];
        var c = ShipperData[0]['ShipperState'];
        var d = ShipperData[0]['ShipperCountry'];
        var e = ShipperData[0]['ShipperZip'];

        $('#BillingAdd').val(ShipperData[0]['ShipperAddress'] +  ShipperData[0]['ShipperCity'])
    }
}
//DATA
{
    var GetMasterData = function () {
        ...
        var tempList = {};
        $.when(RequestServer("CreateInvoiceVietnam.aspx/GetFilterData", _obj)).done(function (response) {
    ...
    var GetProductData = function (invoiceis) {
        ...
        var tempList = {};
        $.when(RequestServer("CreateInvoiceVietnam.aspx/GetProductData", _obj)).done(function (response) {
            tempList = $.parseJSON(response);
        });
        return tempList;
    }
    ...
    var getShipperdata = function () {
        ...
        var tempList = {};
        $.when(RequestServer("CreateInvoiceVietnam.aspx/GetShipperData", _obj)).done(function (response) {
            tempList = $.parseJSON(response);
        });
        return tempList;
    }
    ...
    var UpdateInvoicedata = function (_obj) {
        ...
        $.when(RequestServer("CreateInvoiceVietnam.aspx/UpdateInvoicedata", updateObj)).done(function (response) {
...
//Common
{
    ...
    var Bindrows = function () {
        ...
        $("#tbl_productlst").append("<tr><td><select class='select2 select2-hidden-accessible' id='slt-product'><option value='0'>Choose Product</option></select></td >"
            ...
            + "<td  style='display:none'><input type='text' class='form-control isc-input-txt numeric' id='txtdiscount' style='display:none'/><select id='slt-discount' class='form-control isc-input-slt' style='display:none'><option>%</option><option>$</option></select ></td>"
            + "<td  style='display:none'><input type='text' class='form-control numeric' id='txt_tax'  style='display:none'/></td>"
    ...
    var BindrowsEdit = function (editInvoiceID) {
        ...
        $.each(lstdetail, function (index, item) {
            ...
            $("#tbl-body").append(
                ...
                + "<td style='display:none'><input type='text' value=" + "0" + " class='form-control isc-input-txt numeric' id='txtdiscount' readonly/><select disabled='true' id='slt-discount' class='form-control isc-input-slt'><option>%</option><option>$</option></select ></td>"
                + "<td style='display:none'><input type='text' value=" + "0" + " class='form-control numeric' id='txt_tax' readonly/></td>"
    ...
    var CalculatesuptotalEdit = function () {
    ...
    var BindInvoiceList = function (lstinvoicedetail) {
        ...
        $.each(lstinvoicedetail, function (index, item) {
            ...
            $('#slt-Loading').val(item["PortOfLoading"]);
            $('#slt-Loading').select2();

            $('#slt-Discharge').val(item["PortOfDischarge"]);
            $('#slt-Discharge').select2();
...
//Click 
{
    ...
    var SaveInvoice = function (InvoiceList) {
        ...
        var tempList = {};
        $.when(RequestServer("createInvoiceVietnam.aspx/InsertBill", _obj)).done(function (response) {
            tempList = $.parseJSON(response);
        ...
        if (Isproduct != 0) {
            ...
            if ($("#slt-Shipper").val() != 0 && $("#slt-Consignee").val() != 0 && $("#txt-invoicedate").val() != "") {
                ...
                var _obj = {
                    ...
                    'PortofLoading': parseInt($('#slt-Loading').val()),
                    'PortofDischarge': parseInt($('#slt-Discharge').val()),
    ...
    var GetIvoiceEditBind = function (billId) {
        ...
        var tempList = {};
        $.when(RequestServer("createInvoiceVietnam.aspx/GetInvoicdetails", _obj)).done(function (response) {
            tempList = $.parseJSON(response);
        });
        return tempList;
    }
    ...
    $(document).on('blur', '#txt_quantity1,#txt_price1,#txtdiscount,#txt_tax', function () {
        ...
        if (Product != 0) {
            ...
            var price = parseFloat($(this).closest('tr').find('#txt_price').val() == "" ? 0.00 : $(this).closest('tr').find('#txt_price1').val()).toFixed(2);
    ...
    var SaveCustomer = function () {
        ...
        $.when(RequestServer("createInvoiceVietnam.aspx/SaveCustomer", insertObj)).done(function (response) {
            if (parseInt(response) > 0) {
                $.notify("Customer created successfully!", { position: "right top", className: "success" });
                $("#mp_paid3").hide();
                GoBack()
