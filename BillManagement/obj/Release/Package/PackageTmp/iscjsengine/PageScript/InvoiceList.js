...

{
        ...
        $('#tbl-invoice').DataTable({
            ...
            "ajax": {
                ...
                "dataSrc": function (json) {
                    ...
                    json.data = common.AUF(objData['Invoicelist']);
            ...
            'aoColumns': [
...

                {
                    ...
                    "mData": function (data, type, dataToSet) {
                        var html = '';
                        html += '<h5 >' + (data["TermCode"] == null || data["TermCode"] == '' ? '-' : data["TermCode"]) + '</h5>';
                        return html;
                    }
                },
                {
                ...
                },//TermCode
                {
                    "width": '14.5%',
                    "mData": function (data, type, dataToSet) {
                        var html = '<h5 style="text-align: right; color: #66B050!important;">' + ("$" + (data["Dueamount"] == null || Math.sign(parseFloat(data["Dueamount"])) == -1 ? '0.00' : parseFloat(data["Dueamount"]).toFixed(2).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,"))) + ' </h5>';
                        return html;
                    }
                },

                {
                    ...
                    "mData": function (data, type, dataToSet) {
                        ...
                        html += '<a class="isc-action-badge-td-s1 pad-lft-5 isc-view" title="View" href="#" data-View="' + data["InvoiceId"] + '" ><i class="fa fa-eye"></i></a>';
                       //if (data["BillStatus"] != 2) 
                      //  html += '<a href="#" class="isc-action-badge-td-s1 pad-lft-5" data-toggle="modal" title="Pay Now" data-markaspaid="' + data["InvoiceId"] + '" data-customerid="' + data["Customerid"] + '" data-billstatus="' + data["BillStatus"]+'"> <img src="img/makepayment.png" style="width:19px;" class="isc-mal-pay-icon"></a>'
                        //}
                        // if (data["BillStatus"] == 0) {

                        // html += '<a class="isc-action-badge-td-s1 pad-lft-5" title="Email" href="#" data-emailid="' + data["InvoiceId"] + '"><i class="fa fa-envelope"></i></a>';
                        // html += '<a class="isc-action-badge-td-s1 pad-lft-5" title="Edit"  href="createInvoiceAR.aspx?Invoiceid=' + (data["InvoiceId"] == null ? '' : data["InvoiceId"]) + '"><i class="fa fa-pencil-square-o"></i></a>';
                        //html += '<a class="isc-action-badge-td-s1 pad-lft-5" title="Time Line" data-timeline="' + data["InvoiceId"] + '"  href="#"><i class="fa fa-pencil-square-o"></i></a>';
                        ...
                        //html +=' <li><a href="#MP_Batch" data-toggle="modal" class="isc-action-badge-td-s1 pad-lft-5" title="View">View</a></li>'
                        html += ' <li><a href="#" data-Editinvoice="' + data["InvoiceId"] + '" data-billstatus="' + data["BillStatus"] + '" data-masterinvoice="' + data["MasterInvoice"] +'" class="isc-action-badge-td-s1 pad-lft-5" title="Edit">Edit</a></li>'
                        ...
                        // html +=' <li><a href="#MP_Batch1" data-toggle="modal" class="isc-action-badge-td-s1 pad-lft-5" title="Comments">Comments</a></li>'
                        html += ' </ul>'
                        html += ' </div>'
    ...
    var ConfigurePaginationModel = function (objModel) {
        ...
        obj.Customer = $('select#slt-customer option:selected').val();
    ...
    var ConfigureOrderBy = function (objModel) {
        ...
        switch (columnName) {
            ...
            case 3:
                ...
                break;
            case 4:
                orderBy = "TermCode " + direction;
                break;
            case 5:
                orderBy = "Dueamount " + direction;
                break;
        }
        return orderBy;
    }
...
//data manipulation
{
    ...
    var BindDropDown = function () {
        ...
        var Customerlst = common.AUF($.parseJSON(Invoiceliastdata[0]["Table"]));

        var CostomerHtml = '<option value="0">Choose Customer</option>';
        $.each(Customerlst, function (index, item) {
            CostomerHtml += `<option value="${item.KeyListID}">${item.Value1}</option>`;
        });
        $('#slt-customer').html(CostomerHtml);
    ...
    var BindViewDetails = function (Viewlist) {
        ...
        $.each(Viewlist, function (index, item) {
            ...
            $('#ibl_total').html(parseFloat(item["TOTAL"]).toFixed(2).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,"));
...
//Click Event
{
    ...
    $(document).on('click', '[data-Editinvoice]', function () {
        ...
        else if (masterinvoice != "null")
        {
            ...
            window.location.href = "createInvoiceAR.aspx?Invoiceid=" + invoiceId + "";
        }
    ...
    var BindRrecurringList = function (recurringList) {
        ...
        if (recurringList != undefined && recurringList != null && recurringList.length > 0) {
            ...
            $.each(recurringList, function (index, item) {
                ...
                html += '<a href="createInvoiceAR.aspx?Invoiceid=' + (item["InvoiceId"] == null ? '' : item["InvoiceId"]) + '" class="isc-action-badge-td-s1 pad-lft-5" title="Edit"><i class="fa fa-pencil-square-o"></i></a>';
