...
{
    /// <summary>
    /// Summary description for BillManagement
    /// </summary>
    public class BillManagement : IHttpHandler, System.Web.SessionState.IRequiresSessionState
    {

        public void ProcessRequest(HttpContext context)
        {
            int postStatus = 0;
            List<ResponseAPI> LR_API = new List<ResponseAPI>() { };
            ResponseAPI fileObjects = new ResponseAPI();
            try
            {
                //if (Logger.IsLoggingEnabled())
                //    Logger.Write(new VerboseLogEntry("User Comes to upload the file", 1, "User Requests"));
                if (context.Session["AccountID"] != null && context.Session["Role"] != null)
                {
                    ...
                    if (context.Request.Files.Count > 0)
                    {
                       ...
                       // Logger.Write(new VerboseLogEntry("File Saved directory : " + billDirectory + fileName + "", 1, "User Requests"));
                        string StrExtention = System.IO.Path.GetExtension(billDirectory + fileName);
                        //Logger.Write(new VerboseLogEntry("File Extension : " + StrExtention + "", 1, "User Requests"));
                        ...
                        //Logger.Write(new VerboseLogEntry("File Physical location : " + billDirectory + fileName + "", 1, "User Requests"));
                        string Token = RESTAPI.GetAPIToken();
                        byte[] FileByte = System.IO.File.ReadAllBytes(billDirectory + fileName);
                        string Mimetype = GetMimeType(StrExtention);
                        RESTAPI.GetFiles(Token, FileByte, Mimetype, ref fileObjects);
                    }
                }
                else
                {
            ...
            context.Response.ContentType = "text/json";
            //Logger.Write(new VerboseLogEntry("File Response : " + strFileDetails + "", 1, "User Requests"));
