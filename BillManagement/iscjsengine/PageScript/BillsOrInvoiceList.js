...
//DOM Manipulation
{
    ...
    var BindFilters = function () {
        ...
        BinGroupByDropDown($('#slt-Vgroupby'), $.parseJSON(filterList[4]["Table4"]), 'CategoryName');
        BinGroupByDropDown($('#slt-Hgroupby'), $.parseJSON(filterList[4]["Table4"]), 'StatusName');
        $('#slt-Vgroupby option[value="VendorName"]').prop('selected', 'selected').change();
        $('#slt-Hgroupby option[value="StatusName"]').prop('selected', 'selected').change();
                    ...
                    // Body
                    {
                        if (MatchedItems.length > 0) {
                            $.each(MatchedItems, function (a, b) {
                                ...
                                var approvedAmount = amount - balanceAmount;
                                var isActive = b["IsActive"];
                                if (parseInt(isActive) == 2) {
                                    approvedAmount = "";
                                }
                                ...
                                if (b['Status'] == "50015" || b['Status'] == "50034" || b['Status'] == '50016') {
                                    html += '<h5><a>' + (b["Description"] == null || b["Description"] == "" ? '-' : b["Description"]) + '</a></h5>';
                                }
                                else {
                                    html += '<h5 ><a style="cursor: pointer;" href="UploadBills.aspx?BillID=' + b["BillID"] + '" >' + (b["Description"] == null || b["Description"] == "" ? '-' : b["Description"]) + '</a></h5>';
                                }
                               

                                html += '</td>';
                              //  if (b["InvoiceNumber"] != null && b["InvoiceNumber"] != '') {
                                html += '<td><h5 class="">' + (b["InvoiceNumber"] == null || b["InvoiceNumber"]=='' ? "-" : b["InvoiceNumber"]) + '</h5></td>';
                                //}
                                //else {
                                  //  html += '<td><h5 class="">' - '</h5></td>';
                                //}
                                ...
                                if (moment(today) <= dueDay) {
                                    html += '<td><h5 title="' + (b["DueDate"] == null ? '-' : moment(b["DueDate"]).format('MM/DD/YYYY')) + '">' + (b["DueDate"] == null ? '-' : moment(b["DueDate"]).format('MM/DD/YYYY')) + '</h5></td>';
                                }
                                else {
                                    html += '<td><h5  title="' + (b["DueDate"] == null ? '-' : moment(b["DueDate"]).format('MM/DD/YYYY')) + '" style="color:red !important;">' + (b["DueDate"] == null ? '-' : moment(b["DueDate"]).format('MM/DD/YYYY')) + '</h5></td>';
                                }
                               ...
                               // }
                                ...
                                else if (b['Status'] == "50017") {
                                    html += '<a class="isc-action-badge-td-s1 pad-lft-5" data-view-Bill="' + b["BillID"] + '" title="View Bill" ><i class="fa fa-paperclip"></i></a>';
                                    html += '<a class="isc-action-badge-td-s1 pad-lft-5" data-edit-Bill="' + b["BillID"] + '" title="Edit" href="UploadBills.aspx?BillID=' + b["BillID"] + '"><i class="fa fa-pencil-square-o"></i></a>';
                                    html += '<a class="isc-action-badge-td-s1 pad-lft-5" data-edit-Bill="' + b["BillID"] + '" title="Submit" href="BillSubmit.aspx?BillID=' + b["BillID"] + '"><i class="fa fa-check"></i></a>';
                                }
                                else{
                                html += '<a class="isc-action-badge-td-s1 pad-lft-5 pad-lft-zro" data-view-Bill="' + b["BillID"] + '" title="View Bill" ><i class="fa fa-paperclip"></i></a>';
                                html += '<a class="isc-action-badge-td-s1 pad-lft-5 pad-lft-zro" title="Edit" href="UploadBills.aspx?BillID=' + b["BillID"] + '"><i class="fa fa-pencil-square-o"></i></a>';
                                html += '<a class="isc-action-badge-td-s1 pad-lft-5 pad-lft-zro" data-edit-Bill="' + b["BillID"] + '" title="Submit" href="BillSubmit.aspx?BillID=' + b["BillID"] + '"><i class="fa fa-check"></i></a>';
                                html += '<a class="isc-action-badge-td-s1 pad-lft-5 pad-lft-zro" data-delete-Bill="' + b["BillID"] + '" title="Delete" href="#"><i class="fa fa-trash-o"></i></a>';
                                }
                                html+='</td>';
                                html += '</tr>';
                            });
...
//DATA Manipulation
{
    var GetFilterList = function () {
        ...
        var tempList = {};
        $.when(RequestServer("BillsOrInvoiceList.aspx/GetBillsOrInvoiceListMasterData", _obj)).done(function (response) {
    ...
    var GetBillListData = function () {
        ...
        var tempList = {};
        $.when(RequestServer("BillsOrInvoiceList.aspx/GetInvoiceBillList", _obj)).done(function (response) {
            tempList = $.parseJSON(response);
        });
        return tempList;
    }
