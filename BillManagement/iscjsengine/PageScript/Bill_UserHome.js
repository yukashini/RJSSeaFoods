...
//Dom Manipulation
{
    ...
    var BindKPI = function () {
        debugger
        KPIData = GetUserKPIData();
        var New_Order = [];
        var Packing_Order = [];
        var Delivery_Order = [];
        var Complete_order = [];

        New_Order = common.AUF($.parseJSON(KPIData[0]["Table"]));
        Packing_Order = common.AUF($.parseJSON(KPIData[1]["Table1"]));
        Delivery_Order = common.AUF($.parseJSON(KPIData[2]["Table2"]));
        Complete_order = common.AUF($.parseJSON(KPIData[3]["Table3"]));

        if (New_Order.length > 0) {
            $('#un-Submitted-Amount').html(New_Order[0]["New Order"] == null || New_Order[0]["New Order"] == -1 ? 0 : New_Order[0]["New Order"]);
            $('#un-Submitted-Count').html(New_Order[0]["New Order"] == null || New_Order[0]["New Order"] == -1 ? 0 : New_Order[0]["New Order"]);
        }
        else {
            $('#un-Submitted-Amount').html("0");
        }
        if (New_Order.length > 0) {
            $('#rejected-Amount').html(Packing_Order[0]["Packing Order"] == null || Packing_Order[0]["Packing Order"] == -1 ? 0 : Packing_Order[0]["Packing Order"]);
            $('#rejected-Count').html(Packing_Order[0]["Packing Order"] == null || Packing_Order[0]["Packing Order"] == -1 ? 0 : Packing_Order[0]["Packing Order"]);
        }
        else {
            $('#rejected-Amount').html("0");
        }
        if (New_Order.length > 0) {
            $('#un-Approved-Amount').html(Delivery_Order[0]["Delivery Order"] == null || Delivery_Order[0]["Delivery Order"] == -1 ? 0 : Delivery_Order[0]["Delivery Order"]);
            $('#un-Approved-Count').html(Delivery_Order[0]["Delivery Order"] == null || Delivery_Order[0]["Delivery Order"] == -1 ? 0 : Delivery_Order[0]["Delivery Order"]);
        }
        else {
            $('#un-Approved-Amount').html("0");
        }
        if (New_Order.length > 0) {
            $('#total-Amount').html(Complete_order[0]["Complete order"] == null || Complete_order[0]["Complete order"] == -1 ? 0 : Complete_order[0]["Complete order"]);
            $('#total-Amount-Count').html(Complete_order[0]["Complete order"] == null || Complete_order[0]["Complete order"] == -1 ? 0 : Complete_order[0]["Complete orderr"]);
        }
        else {
            $('#total-Amount').html("0");
        }
...
//Data Manipulation
{
    ...
    var GetUserKPIData = function (obj) {
        var tempList = {};
        $.when(RequestServer("Bill_UserHome.aspx/GetInvoiceData")).done(function (response) {
            tempList = $.parseJSON(response);
        });
        return tempList;
    }
