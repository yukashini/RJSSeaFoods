...
//
{
    var DeleteShipper = function () {

        var obj = {
            'ShipperId': parseInt(deleteShipperID)
        }
        var tempList = {};
        $.when(RequestServer("InvoiceShipper360.aspx/DeleteShipper", obj)).done(function (response) {
...
//Click
{
    ...
    $(document).on('click', '[data-delete-Shipper]', function () {

        deleteShipperID = $(this).attr('data-delete-Shipper');
        if (deleteShipperID != null) {
            $('#mp_Shipper_Delete').show();
        }
    });
